...
from models.ResNetBranch import *
from utils import *
from os import path
        
def create_model(depth=4, branch=1, stage1_weights=False, dataset=None, log_dir=None, test=False, *args):
    
    print('Loading Scratch ResNet 50 Feature Model.')
    resnet50 = ResNet(Bottleneck, [3, 4, 6, 3], branch=branch,depth=depth)

    if not test:
        if stage1_weights:
            assert(dataset)
            print('Loading %s Stage 1 ResNetBranch 50 Weights.' % dataset)
